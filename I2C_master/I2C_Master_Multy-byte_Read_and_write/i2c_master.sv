...
clock_divider u_dclock (.clk(clk), .out_clk(clk_4));

start_i2c u_start (.stat_en(start_en), .clk(clk_4), .rst(rst), .sda(sda), .sck(sck), .done(start_done));

repeated_start u_rep_s (.stat_en(rep_start_en), .clk(clk_4), .rst(rst), .sda(sda), .sck(sck), .done(rep_start_done));

read_byte u_read (.stat_en(read_en), .clk(clk_4), .rst(rst), .rx_byte(rx_byte), .sda(sda), .sck(sck), .done(read_done));

send_byte u_byte (.stat_en(byte_en), .clk(clk_4), .rst(rst), .tx_byte(current_tx_byte), .sda(sda), .sck(sck), .done(byte_done));

ack u_ack (.stat_en(ack_en), .clk(clk_4), .rst(rst), .sda(sda), .ack(ack_received), .sck(sck), .done(ack_done));

master_ack u_mack (.clk(clk_4), .rst(rst), .stat_en(m_ack_en), .sda(sda), .sck(sck), .done(m_ack_done));

master_nack u_mnack (.clk(clk_4), .rst(rst), .stat_en(m_nack_en), .sda(sda), .sck(sck), .done(m_nack_done));

stop_i2c u_stop (.stat_en(stop_en), .clk(clk_4), .rst(rst), .sda(sda), .sck(sck), .done(stop_done));
    ...
    if (!rst) begin
        ...
        case (sts)
            IDLE: begin
                ...
                if (start_pulse_clk4) begin
                    sts <= START_M;
                end
            end
            START_M: begin
                start_en <= 1;
                rd_cnt   <= (rd_len == 0) ? 1 : rd_len;
                wr_cnt   <= (wr_len == 0) ? 1 : wr_len;
                if (start_done) begin
                    start_en <= 0;
                    sts      <= LOAD_SLV_ADDR_W;
                end
            ...
            LOAD_TX_DATA: begin
                current_tx_byte <= tx_buffer[tx_index];
                sts             <= SEND_TX_DATA;
            end
            SEND_TX_DATA: begin
                byte_en <= 1;
