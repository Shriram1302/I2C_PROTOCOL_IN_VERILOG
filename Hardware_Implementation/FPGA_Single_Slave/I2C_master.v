module i2c_master (
    ...
    input            start,        // Active pulse / trigger
    input            rw,           // 1 = Read, 0 = Write
    input      [6:0] slave_addr,   // Targeted Slave Address
    input      [7:0] reg_addr,     // Register/Command Address
    input      [7:0] tx_data,      // Data to transmit (Write mode)
    inout            sda,          // I2C SDA Line
    inout            sck,          // I2C SCK Line
    output reg [7:0] rx_data,      // Data received (Read mode)
    output reg       done,         // Transaction Done Flag
    output reg       ack_error     // ACK Failure Flag
);

    // Wire Declarations for Sub-Modules
    wire clk_4;
    wire start_done;
    wire rep_start_done;
    wire read_done;
    ...
    // Synchronizer logic for the 'start' trigger
    reg start_toggle;
    ...
    // SCK Holding Logic to safely transition addresses/bytes
